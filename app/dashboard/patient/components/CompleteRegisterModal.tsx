// app/dashboard/patient/components/CompleteRegisterModal.tsx
'use client';

import { useState } from 'react';
import { X, Save, Calendar } from 'lucide-react';
import { submitRegisterEntry } from '@/app/actions/registerEntries';
...
export default function CompleteRegisterModal({ assignment, onClose }: Props) {
  const [formData, setFormData] = useState<RegisterEntryData>({});
  ...
  const fields = assignment.auto_register.fields?.fields || [];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await submitRegisterEntry({
      ...
      if (result.error) {
        ...
        setTimeout(() => {
          onClose();
          window.location.reload(); // Recargar para mostrar nueva entrada
        }, 1500);
      }
    } catch (err) {
          ...
          <div className="p-6 max-h-[calc(90vh-200px)] overflow-y-auto space-y-6">
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                placeholder="¿Algo más que quieras añadir sobre esta entrada?"
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent outline-none resize-none"
              />
            </div>
